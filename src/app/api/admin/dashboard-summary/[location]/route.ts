...
export async function GET(req: Request, { params }: { params: { location: string } }) {
  ...
  const [ledger, requests, songs] = await Promise.all([
    prisma.creditLedger.findMany({ where: ledgerWhere, select: { delta: true, reason: true, createdAt: true } }),
    prisma.request.findMany({ where: { locationId: location.id, createdAt: { gte: start } }, include: { song: { select: { id: true, title: true, artist: true } } }, orderBy: { createdAt: "desc" }, take: 10 }),
  ...
  for (const request of requests) { const current = requestCounts.get(request.songId) || { title: request.song.title, artist: request.song.artist, count: 0 }; current.count += 1; requestCounts.set(request.songId, current); }
